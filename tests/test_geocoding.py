...
def geocoding_service(mock_nominatim_instance):
    ...
    with patch('accessiweather.geocoding.Nominatim', return_value=mock_nominatim_instance) as mock_nominatim_class:
...
def test_geocode_address_zip5(geocoding_service, mock_location):
    ...
    assert result == (SAMPLE_LAT, SAMPLE_LON, SAMPLE_DISPLAY_NAME)
    geocoding_service._mock_geolocator_instance.geocode.assert_called_once_with(f"{SAMPLE_ZIP_5}, USA")
...
def test_geocode_address_service_error(geocoding_service):
    ...
    geocoding_service._mock_geolocator_instance.geocode.side_effect = GeocoderServiceError("Service Error")
