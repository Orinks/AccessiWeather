...
MOCK_CONFIG_DIR = "/home/user/.accessiweather"
MOCK_LOG_DIR = Path(MOCK_CONFIG_DIR) / "logs"
...
def test_setup_logging_directory_creation():
    ...
    mock_console = MagicMock(level=logging.NOTSET)
    mock_file = MagicMock(level=logging.NOTSET)

    with (
        patch("accessiweather.logging_config.get_config_dir", return_value=MOCK_CONFIG_DIR),
        ...
        # mkdir called with parents=True, exist_ok=True for logs subfolder
...
def test_setup_logging_console_handler():
    ...
    with (
        ...
        patch("logging.Formatter", return_value=mock_formatter),
        patch("logging.StreamHandler", return_value=mock_console) as mock_stream,
        patch("accessiweather.logging_config.get_config_dir", return_value=MOCK_CONFIG_DIR),
        patch("pathlib.Path.mkdir"),
        patch("logging.handlers.RotatingFileHandler", return_value=mock_file),
        patch("logging.FileHandler._open"),
...
def test_setup_logging_file_handler():
    ...
    with (
        ...
        patch("logging.Formatter", return_value=mock_formatter),
        patch("logging.StreamHandler", return_value=mock_console),
        patch("logging.handlers.RotatingFileHandler", return_value=mock_file) as mock_file_handler,
        patch("accessiweather.logging_config.get_config_dir", return_value=MOCK_CONFIG_DIR),
        patch("pathlib.Path.mkdir"),
        patch("logging.FileHandler._open", return_value=mock_stream),
    ):
        setup_logging(log_level=logging.INFO)
