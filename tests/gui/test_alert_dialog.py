    ...
    "features": [
        {
            ...
            "properties": {
                ...
                "event": "Test Event",
                "parameters": {
                    "NWSheadline": ["This is a test statement from NWSheadline"]
                }
            }
        }
    ]
...
def test_process_alerts_preserves_parameters():
    ...
    processed_alerts = weather_service.process_alerts(SAMPLE_ALERT_WITH_HEADLINE)
...
def test_alert_details_dialog_with_nwsheadline(mock_wx_dialog):
    ...
    with patch.object(AlertDetailsDialog, '__init__', return_value=None):
        ...
        statement = processed_alerts[0].get("parameters", {}).get(
            "NWSheadline", ["No statement available"]
        )[0]
...
def test_alert_details_dialog_without_nwsheadline(mock_wx_dialog):
    ...
    # Process the sample alert data
    processed_alerts = weather_service.process_alerts(SAMPLE_ALERT_WITHOUT_HEADLINE)
...
def test_alert_details_dialog_with_empty_parameters(mock_wx_dialog):
    ...
    with patch.object(AlertDetailsDialog, '__init__', return_value=None):
        ...
        # Manually call the code that would extract the statement
        statement = alert_data.get("parameters", {}).get(
            "NWSheadline", ["No statement available"]
        )[0]
...
def test_weather_app_on_alerts_fetched_uses_processed_alerts():
    """Test that _on_alerts_fetched uses processed alerts from notification service for UI display."""
    # Create mock objects
    ...
    mock_notification_service.notify_alerts.assert_called_once_with(processed_alerts, len(processed_alerts))
