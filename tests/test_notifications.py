...
def mock_datetime():
    ...
    with patch('accessiweather.notifications.datetime') as mock_dt, \
...
def test_process_alerts_new(notifier, mock_toaster, mock_datetime):
    ...
    processed = notifier.process_alerts(SAMPLE_ALERTS_DATA)
    
    assert len(processed) == 1
    assert processed[0]["id"] == SAMPLE_ALERT["id"]
...
def test_process_alerts_mixed(notifier, mock_toaster, mock_datetime):
    ...
    processed = notifier.process_alerts(SAMPLE_ALERTS_DATA_MULTIPLE)
...
def test_process_alerts_empty(notifier, mock_toaster):
    ...
    processed = notifier.process_alerts({"features": []})
