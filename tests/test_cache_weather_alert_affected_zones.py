"""
Tests for alert coverage metadata serialization in the cache layer.
...
- Round-trip through ``_serialize_alert`` / ``_deserialize_alert`` preserves
  coverage fields exactly.
- Legacy cache entries (written before Unit 9) deserialize with
  empty coverage metadata and never raise.
"""
...
def test_alert_coverage_metadata_round_trip() -> None:
    """Populated zone and SAME metadata survives serialize -> deserialize unchanged."""
    alert = WeatherAlert(
...
def test_affected_zones_omitted_when_empty() -> None:
    """Empty coverage metadata is omitted from the payload — legacy shape."""
    alert = WeatherAlert(
        title="Test Alert",
        description="Body",
    )
    ...
    assert "same_codes" not in payload


def test_legacy_cache_entry_deserializes_with_empty_zones() -> None:
    """A legacy cache dict deserializes with empty coverage metadata."""
    legacy_payload = {
        "title": "Flood Warning",
