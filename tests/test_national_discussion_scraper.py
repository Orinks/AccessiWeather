...
from accessiweather.services.national_discussion_scraper import (
    _last_request_time,  # Import for patching
    fetch_spc_day1,
...
def clean_last_request_time():
    ...
    with patch.dict('accessiweather.services.national_discussion_scraper._last_request_time', clear=True):
        yield

# --- WPC Tests ---

def test_fetch_wpc_success(mock_requests, mock_bs4, mock_sleep):
...
def test_fetch_wpc_no_pre_tag(mock_requests, mock_bs4, mock_sleep):
...
def test_fetch_wpc_request_error(mock_requests, mock_sleep):
...
def test_fetch_wpc_rate_limit(mock_requests, mock_bs4, mock_sleep, mock_time, clean_last_request_time):
...
def test_fetch_spc_success_with_discussion(mock_requests, mock_bs4, mock_sleep):
...
def test_fetch_spc_success_no_discussion_marker(mock_requests, mock_bs4, mock_sleep):
    """Test successful SPC discussion fetch without ...DISCUSSION... marker."""
    mock_requests.return_value.text = "<html><body><pre>Full text without marker</pre></body></html>"
...
def test_fetch_spc_no_pre_tag(mock_requests, mock_bs4, mock_sleep):
...
def test_fetch_spc_request_error(mock_requests, mock_sleep):
...
def test_fetch_spc_rate_limit(mock_requests, mock_bs4, mock_sleep, mock_time, clean_last_request_time):
...
def test_get_national_discussion_summaries_success():
    ...
    with patch('accessiweather.services.national_discussion_scraper.fetch_wpc_short_range') as mock_wpc:
        with patch('accessiweather.services.national_discussion_scraper.fetch_spc_day1') as mock_spc:
