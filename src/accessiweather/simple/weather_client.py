...
class WeatherClient:
    ...
    async def get_weather_data(self, location: Location) -> WeatherData:
        ...
        try:
            ...
            forecast = await self._get_nws_forecast(location)
    ...
    async def _get_nws_current_conditions(self, location: Location) -> Optional[CurrentConditions]:
    ...
    async def _get_nws_forecast(self, location: Location) -> Optional[Forecast]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                return self._parse_nws_forecast(forecast_data)
                
        except Exception as e:
            logger.error(f"Failed to get NWS forecast: {e}")
            return None
    
    async def _get_nws_alerts(self, location: Location) -> Optional[WeatherAlerts]:
    ...
    async def _get_openmeteo_current_conditions(self, location: Location) -> Optional[CurrentConditions]:
    ...
    async def _get_openmeteo_forecast(self, location: Location) -> Optional[Forecast]:
    ...
    def _parse_nws_current_conditions(self, data: dict) -> CurrentConditions:
        ...
        return CurrentConditions(
            ...
            pressure_in=self._convert_pa_to_inches(props.get("barometricPressure", {}).get("value")),
    ...
    # Utility methods
    def _convert_mps_to_mph(self, mps: Optional[float]) -> Optional[float]:
        """Convert meters per second to miles per hour."""
        return mps * 2.237 if mps is not None else None
    
    def _convert_pa_to_inches(self, pa: Optional[float]) -> Optional[float]:
        """Convert pascals to inches of mercury."""
        return pa * 0.0002953 if pa is not None else None
    
    def _convert_f_to_c(self, fahrenheit: Optional[float]) -> Optional[float]:
    ...
    def _weather_code_to_description(self, code: Optional[int]) -> Optional[str]:
