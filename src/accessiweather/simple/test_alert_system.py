...
def test_alert_model():
    ...
    assert alert.get_severity_priority() == 3, f"Expected priority 3 for Moderate, got {alert.get_severity_priority()}"
...
def test_alert_manager():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(notifications2) == 0, f"Expected 0 notifications on repeat, got {len(notifications2)}"
...
async def test_notification_system():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(mock_notifier.notifications_sent) == 1, "Mock notifier should have 1 notification"
        
        notification = mock_notifier.notifications_sent[0]
        assert "Test Alert" in notification["title"], f"Title should contain 'Test Alert', got '{notification['title']}'"
