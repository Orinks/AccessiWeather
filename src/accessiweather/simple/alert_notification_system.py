...
class AlertNotificationSystem:
    ...
    def __init__(self, alert_manager: AlertManager, notifier: Optional[SafeDesktopNotifier] = None):
        ...
        try:
            ...
            for alert, reason in notifications_to_send:
                try:
                    ...
                    logger.error(f"Failed to send notification for alert {alert.get_unique_id()}: {e}")
            
            logger.info(f"Sent {notifications_sent} of {len(notifications_to_send)} alert notifications")
    ...
    async def _send_alert_notification(self, alert: WeatherAlert, reason: str) -> bool:
        ...
        try:
            ...
            template = self.notification_templates.get(reason, self.notification_templates["new_alert"])
            ...
            message = template["message_format"].format(
                ...
                description=alert.description[:100] + "..." if len(alert.description or "") > 100 else alert.description or "",
                severity=alert.severity or "Unknown",
