...
class MainWindowCommandMixin:
    ...
    def _on_debug_simulate_radio_auto_tune(self) -> None:
        """Inject a SAME-backed alert through the full event path for radio auto-tune."""
        from ..alert_lifecycle import AlertChange, AlertChangeKind, AlertLifecycleDiff
        from ..models.alerts import WeatherAlert
        from ..models.weather import WeatherAlerts, WeatherData
        from ..noaa_radio.alert_auto_tune import county_zone_to_same_code
        ...
        if not getattr(settings, "auto_tune_weather_radio_alerts", False):
            wx.MessageBox(
                "Enable weather radio auto-tune in Settings, Audio before running this test.",
                "Debug: Weather Radio Auto-Tune",
                wx.OK | wx.ICON_INFORMATION,
            )
            return

        county_zone = getattr(location, "county_zone_id", None)
        same_code = county_zone_to_same_code(county_zone)
        if not county_zone or not same_code:
            wx.MessageBox(
                "The current location does not have NWS county zone metadata yet.\n\n"
                "Refresh or re-save a US location so AccessiWeather can populate its "
                "county zone before testing weather radio auto-tune.",
                "Debug: Weather Radio Auto-Tune",
                wx.OK | wx.ICON_WARNING,
            )
            return
        ...
        fake_alert = WeatherAlert(
            title="Tornado Warning",
            description=(
                "This is a simulated SAME-backed alert injected via the debug menu "
                "to test NOAA Weather Radio auto-tune."
            ),
            severity="Extreme",
            urgency="Immediate",
            certainty="Observed",
            event="Tornado Warning",
            headline=f"DEBUG: Simulated Tornado Warning for {location.name}",
            areas=[getattr(location, "name", "Current location")],
            id=f"debug-radio-auto-tune-{county_zone.lower()}",
            message_type="Alert",
            affected_zones=[county_zone],
        ...
        self._on_notification_event_data_received(mock_data)
        wx.MessageBox(
            "Injected a SAME-backed alert for weather radio auto-tune.\n\n"
            "If a matching WeatherIndex station is available, radio playback should start "
            "in the background and stop after the configured duration.",
            "Debug: Weather Radio Auto-Tune",
            wx.OK | wx.ICON_INFORMATION,
        )
