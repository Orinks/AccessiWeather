...
def build_current_weather_payload(
    ...
    *,
    ...
    wind_speed_unit_preference: str | None = "auto",
    ...
    wind_unit_system = resolve_wind_display_unit_system(
        wind_speed_unit_preference,
        temperature_preference=temperature_unit_preference,
        location=location,
    )
    ...
    weather_dict: dict[str, Any] = {
        ...
        "wind_speed": _resolve_wind_speed_for_prompt(current, unit_pref, wind_unit_system),
        "wind_speed_unit": _resolve_wind_speed_unit_label(unit_pref, wind_unit_system),
        ...
        weather_dict["forecast_periods"] = [
            {
                ...
                "temperature_text": format_temperature(
                    period.temperature if period.temperature_unit == "F" else None,
                    unit=unit_pref,
                    temperature_c=period.temperature if period.temperature_unit == "C" else None,
                ),
                "short_forecast": period.short_forecast,
                "wind_speed": format_wind_speed(
                    period.wind_speed_mph, unit=unit_pref, unit_system=wind_unit_system
                )
                if isinstance(period.wind_speed_mph, (int, float))
                else period.wind_speed,
