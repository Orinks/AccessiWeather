...
class ReorderLocationsDialog(wx.Dialog):
    ...
    def __init__(self, parent, app: AccessiWeatherApp):
        ...
        dialog_sizer.Add(panel, 1, wx.EXPAND)
        dialog_sizer.Add(
            self.CreateSeparatedButtonSizer(wx.OK | wx.CANCEL), 0, wx.ALL | wx.EXPAND, 10
        )
        self.SetSizer(dialog_sizer)
    ...
    def _move_selection(self, direction: int) -> None:
        ...
        if frame is not None:
            frame.set_status(f"{moved_name} moved to position {new_index + 1}.")
