...
class WeatherApp(wx.Frame):
    ...
    def _check_update_complete(self):
        ...
        if self._forecast_complete and self._alerts_complete:
            ...
            log_msg = (
                "Both forecast and alerts fetch complete. "
    ...
    def _on_discussion_error(self, error_message, name, loading_dialog):
        ...
        # Show error message
        msg = (
            f"Failed to fetch forecast discussion for {name}:\n\n"
            f"{error_message}"
        )
    ...
    def OnViewAlert(self, event):
        ...
        if selected_index < len(self.current_alerts):
            ...
            wx.MessageBox(
                details, "Alert Details", wx.OK | wx.ICON_INFORMATION
            )
        else:
            logger.error(
    ...
    def OnSettings(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            self.config["api_settings"][API_CONTACT_KEY] = new_settings[
                API_CONTACT_KEY
            ]
            self.config["settings"][UPDATE_INTERVAL_KEY] = new_settings[
                UPDATE_INTERVAL_KEY
            ]
            self.config["settings"][ALERT_RADIUS_KEY] = new_settings[
                ALERT_RADIUS_KEY
            ]
    ...
    def _save_config(self):
        ...
        try:
            ...
            log_msg = (
                f"Failed to save configuration to " f"{self._config_path}: {e}"
            )
