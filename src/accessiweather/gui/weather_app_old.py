...
from .settings_dialog import (ALERT_RADIUS_KEY, API_CONTACT_KEY,
                              UPDATE_INTERVAL_KEY, SettingsDialog)
...
class WeatherApp(wx.Frame):
    ...
    def _check_update_complete(self):
        ...
        if self._forecast_complete and self._alerts_complete:
            ...
            log_msg = (
                "Both forecast and alerts fetch complete. " "Refresh button re-enabled."
            )
    ...
    def OnViewAlert(self, event):
        ...
        if selected_index < len(self.current_alerts):
            ...
            wx.MessageBox(
                "Error retrieving alert details.", "Error", wx.OK | wx.ICON_ERROR
            )
    ...
    def _save_config(self):
        ...
        try:
            ...
            wx.MessageBox(
                f"Error saving settings: {e}", "Save Error", wx.OK | wx.ICON_ERROR, self
            )
