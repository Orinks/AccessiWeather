...
from accessiweather.current_location import CurrentCoordinates, location_from_coordinates  # noqa: E402
...
MANAGER_ROUTES = [
    ...
    (client_search("Zürich", 5), 200, ZURICH),
    (url(CENSUS, {"address": "4600 Silver Hill Rd, Washington, DC", "benchmark": "Public_AR_Current",
                  "format": "json"}), 200, CENSUS_MATCH),
...
REVERSE_CASES = {
    "nws_city_state": (39.9526, -75.1652, [
        (NWS, 200, {"properties": {"timeZone": "America/New_York", "relativeLocation": {
            "properties": {"city": " Philadelphia ", "state": "PA"}}}}),
    ]),
    "nws_city_only": (61.2181, -149.9003, [
        (NWS, 200, {"properties": {"relativeLocation": {"properties": {"city": "Anchorage"}}}}),
    ]),
    "nominatim_city": (48.8566, 2.3522, [
        (NWS, 404, {"properties": {}}),
        (NOMINATIM, 200, {"display_name": "Paris, Île-de-France, France", "address": {
            "city": "Paris", "state": "Île-de-France", "country": "France", "country_code": "fr"}}),
    ]),
    "nominatim_country_only": (0.0, -160.0, [
        (NWS, 200, {"properties": {"relativeLocation": {"properties": {"city": ""}}}}),
        (NOMINATIM, 200, {"address": {"country": "Kiribati", "country_code": "ki"}}),
    ]),
    "nominatim_display_name": (10.0, 10.0, [
        (NWS, 404, {}),
...
def main() -> None:
    ...
    for mode, source, query in SERVICE_CASES:
        ...
        common.write(
            ...
            {"mode": mode, "data_source": source, "query": query,
             "exchanges": router.exchanges(), "output": output},
        )
    ...
    for query, limit in MANAGER_CASES:
        ...
        common.write(
            ...
            {"query": query, "limit": limit, "exchanges": router.exchanges(),
             "output": locations_json(output) if output is not None else None, "error": error},
        )
    ...
    common.write(AREA, "pure_functions", {
        "street_address": {q: manager._looks_like_street_address(q) for q in [
            "1600 Pennsylvania Ave NW, Washington, DC", "10 Downing Street", "221B Baker St",
            "221 Baker, London", "New York, NY", "10001", "Route 66", "5th Avenue",
            "12 main st apt 4", "Apartment 5, Main", "  42 Wallaby Way, Sydney"]},
        "format_coordinates": [
            [lat, lon, manager.format_coordinates(lat, lon)]
            for lat, lon in [(40.7128, -74.006), (-33.8688, 151.2093), (0.0, 0.0), (51.50005, -0.12785)]
        ],
        "distance": [
        ...
        "validate_us_nws": [
            ...
            for lat, lon in [(40.0, -75.0), (51.5, -0.1), (61.2, -149.9), (52.0, 175.0),
                             (21.3, -157.8), (18.2, -66.5), (13.4, 144.8), (95.0, 0.0)]
        ],
        "zip_codes": {z: service_nws.is_zip_code(z) for z in
                      ["12345", "12345-6789", "1234", "123456", "12345-678", "abcde", "12345\n"]},
        "fallback_queries": {n: om_client._build_fallback_queries(n) for n in
                             ["Zurich", "Tromso", "Paris, France", "Munchen", "Sao Paulo", "Reykjavik",
                              "New York", "ALESUND", "Cafe"]},
        "normalize_text": {t: om_client._normalize_text(t) for t in
                           ["  Zürich,  CH ", "São Paulo", "Straße", "Ålesund", "Kraków!!", "東京"]},
        "nominatim_name": [
            [data, manager._format_nominatim_location_name(data)] for data in [
                {"address": {"town": "Hamlet", "province": "Hamlet", "country": "Ruritania"}},
                {"address": {"suburb": "", "county": "Kent", "state_district": "SE", "country": "UK"}},
        ...
        "geocoding_result": [
            [data, common.to_jsonable(manager._parse_geocoding_result(data))] for data in [
                {"name": "Paris", "admin1": "Île-de-France", "country": "France", "country_code": "fr",
                 "latitude": 48.85, "longitude": 2.35},
                {"name": "Springfield", "admin1": "Springfield", "country": "United States",
                 "latitude": 39.8, "longitude": -89.6},
                {"latitude": "1.5", "longitude": 2, "country": "United States of America"},
            ]
        ],
        "sorting": [
            [order, anchor.name if anchor else None,
             [loc.name for loc in sort_locations_for_display(places, order, anchor=anchor)]]
            for order, anchor in [("alphabetical", None), ("manual", None),
                                  ("nearest_current", places[2]), ("nearest_current", places[3]),
                                  ("nearest_current", None), ("bogus", None)]
        ],
        "current_location": [
            common.to_jsonable(location_from_coordinates(CurrentCoordinates(lat, lon), name=name))
            for lat, lon, name in [(40.7128, -74.006, None), (43.65, -79.38, None),
                                   (43.65, -79.38, "Toronto"), (51.5, -0.12, "")]
        ],
    })
