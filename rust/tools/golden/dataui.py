            ...
            out.append(
                {
                    ...
                    "name": child.GetName(),
                    **look,
                    "wrap": getattr(child, "_golden_wrap", None),
                }
            )
        else:
...
POINTS = [
    ...
    point(
        2, precipitation_intensity=0.2, precipitation_probability=0.125, precipitation_type="rain"
    ),
    point(
        3,
        precipitation_intensity=0.05,
        precipitation_intensity_error=0.1,
        precipitation_probability=0.135,
        precipitation_type="rain",
    ),
    point(
        4,
        precipitation_intensity=1.23456,
        precipitation_intensity_error=0.2,
    ...
    point(
        ...
        precipitation_intensity_error=0.01,
    ...
    point(
        m,
        precipitation_intensity=0.1 * (m % 7),
        precipitation_probability=(m % 10) / 10,
...
PRECIP_CASES = [
    ...
    ("no zone", "Somewhere", None, MinutelyPrecipitationForecast(summary=None, points=POINTS[2:6])),
    (
        "bad zone",
        "Mars",
        "Mars/Olympus_Mons",
...
AQ_HOURS = [
    aq_hour(
        ...
        pm10=15.0,
        ozone=61.349,
        nitrogen_dioxide=12.05,
        sulphur_dioxide=1.0,
        carbon_monoxide=210.55,
    ),
...
UV_HOURS = [
    HourlyUVIndex(
        timestamp=datetime(2026, 9, 25, 6 + h, 0, tzinfo=NY), uv_index=v, category="Moderate"
    )
    for h, v in enumerate([0.0, 0.4, 1.5, 2.5, 3.49, 5.5, 7.5, 8.2, 9.0, 10.5, 11.0, 6.0, 3.0, 1.0])
]
...
ENVIRONMENTS = [
    ...
    (
        ...
        EnvironmentalConditions(
            ...
            air_quality_pollutant="OZONE",
            pollen_index=3.0,
            uv_category="Moderate",
            updated_at=datetime(2026, 1, 5, 0, 30, tzinfo=UTC),
            sources=["Open-Meteo"],
        ),
    ),
    (
        ...
        EnvironmentalConditions(
            ...
            air_quality_source="Open-Meteo Air Quality",
            hourly_air_quality=[aq_hour(12, 150)],
            uv_index=3.2,
        ),
    ),
    (
        "no2 dominant",
        EnvironmentalConditions(
            air_quality_index=99.0,
            air_quality_category="Moderate",
            air_quality_pollutant="no2",
            hourly_air_quality=[aq_hour(3, 99, nitrogen_dioxide=40.0, carbon_monoxide=300.0)],
            uv_category="Unknown Category",
            hourly_uv_index=UV_HOURS[:3],
        ),
    ),
    ...
    (
        "aq only",
        EnvironmentalConditions(
            air_quality_index=301.5, air_quality_category="Hazardous", air_quality_pollutant="Smoke"
        ),
    ),
    (
        "sources airnow",
        EnvironmentalConditions(
            air_quality_index=12.0,
    ...
    (
        ...
        EnvironmentalConditions(
            ...
            air_quality_category="Mystery",
            air_quality_source="  ",
            sources=["Pirate", "Pirate", "Open-Meteo"],
...
SIGMETS = [
    {
        "event": "Convective SIGMET 45E",
        "description": "  Embedded thunderstorms.  ",
        "startTime": "2026-09-25T14:55:00Z",
        "endTime": "2026-09-25T16:55:00Z",
    },
    {
        "event": "",
        "name": "SIGMET NOVEMBER 3",
        "summary": "Severe turbulence " * 20,
...
AVIATION_CASES = [
    ...
    (
        ...
        "KORD",
        AviationData(
            raw_taf="TAF KORD", decoded_taf="", airport_name="", station_id="", active_cwas=CWAS
        ),
    ),
]
...
def aviation_state(dlg) -> dict:
    ...
    return {
        ...
        "rows": [[items.GetItemText(r, c) for c in range(4)] for r in range(items.GetItemCount())],
...
def aviation_cases(parent) -> dict:
    ...
    try:
        ...
        for code in CODES:
            ...
            validation.append({"input": code, "valid": valid, "status": None if valid else status})
        fetches = []
        for name, code, aviation in AVIATION_CASES:
            dlg._on_fetch_complete(code, aviation)
            fetches.append(
                {"name": name, "code": code, "aviation": aviation, **aviation_state(dlg)}
            )
