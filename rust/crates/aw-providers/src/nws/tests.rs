...
fn points_503_is_retried_then_raised() {
    ...
    let http = FixtureClient::new().with_transport_error(POINTS);
...
fn text_product_transport_error_is_a_fetch_error() {
    let http = FixtureClient::new().with_transport_error(AFD_LIST);
    assert!(matches!(
...
fn alert_failures() {
    ...
    let http = FixtureClient::new().with_transport_error(ALERTS);
...
fn cancel_references_query_and_failure() {
    ...
    let http = FixtureClient::new().with_transport_error("https://api.weather.gov/alerts");
